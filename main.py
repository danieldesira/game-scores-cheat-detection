...
def insert_score_in_db(score: Score, rule_sheet: dict):
    ...
    with connection.cursor() as cursor:
        cursor.execute(
            ...
            (score.compute_score(rule_sheet), score.level, score.timestamp, score.player_id, score.outcome, score.duration)
        )
...
def main():
    ...
    while True:
        ...
        if new_score is not None:
            ...
            print(f"{score.level}|{score.duration}|{score.player_id}|{score.outcome}|{score.timestamp}|{score.remaining_resets}")
            try:
